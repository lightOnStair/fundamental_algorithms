...
def insert_floor(floortree,floor):
    prev = None
    cur = floortree.root
    while cur != None:
        prev = cur
        if floor.floor_num < cur.floor_num:
            cur = cur.left
        else:
            cur = cur.right
    floor.parent = prev
    if prev == None:
        floortree.root = floor
...
def delete_floor(floortree,floor):
    if floor.left == None:
        trans(floortree,floor,floor.right)
    elif floor.right == None:
        trans(floortree,floor,floor.left)
    else:
        suc = tminimum(floor.right)
        if suc.parent != floor:
            trans(floortree,suc,suc.right)
            suc.right = floor.right
            suc.right.parent = suc
        trans(floortree,floor,suc)
        suc.left = floor.left
        suc.left.parent =suc
...
def find_nearest_unhaunted(floortree,floornum):
    cur = floortree.root
    prev = None
    close = floortree.root
    temp = 0
    while floornum != cur.floor_num:
        prev = cur
        if floornum < cur.floor_num:
            cur = cur.left
        else:
            cur = cur.right
        if cur == None:
            break
        if abs(prev.floor_num - floornum) > abs(cur.floor_num - floornum):
            temp = cur
        else:
            temp = prev
        if  abs(temp.floor_num - floornum) < abs(close.floor_num - floornum):
            close = temp
    if cur == None:
        cur = close
    return cur
