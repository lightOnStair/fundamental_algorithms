...
def heapify(heap,i):
    ...
    l = i*2+1
    r = i*2+2
    if l<len(heap) and heap[i].get_dist() > heap[l].get_dist():
        small = l
    if r<len(heap) and heap[small].get_dist() > heap[r].get_dist():
        small = r
    if small != i:
...
def getParent(i):
    parent_index = (i-1) // 2
    return parent_index
# actually decrease. make no change when copying from former PA for convinience.
def increase_task_priority(heap,i,new_priority):
    if new_priority > heap[i].get_dist():
        return None
    heap[i].set_dist(new_priority)
    while i>0 and heap[getParent(i)].get_dist() > heap[i].get_dist():
...
def relax(u,v,q):
    if v.get_dist() > u.get_dist() + u.jump_times[v.index]:
        ...
        i = q.index(v)
        increase_task_priority(q,i,v.get_dist())
        v.prev = u
...
def hyperspace_jump_route(stars,matrix):
...
#   self.visited: optional instance variable: may be useful for array PQ implementation
#
...
try:
    for i in range(len(correct)):
        ...
        if enable_turtle:
            ...
            print()
        route = hyperspace_jump_route(list(star_lists[i]),star_maps[i].jump_times)
